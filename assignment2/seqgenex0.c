...
void *Service_1(void *threadp)
{
    ...
    syslog(LOG_CRIT, "[COURSE:1][ASSIGNMENT:1] S1: start on cpu=%d @ sec=%lf\n", sched_getcpu(), current_time);
    ...
    while (!abortS1)
    {
        ...
        current_time = getTimeMsec();
        syslog(LOG_CRIT, "[COURSE:1][ASSIGNMENT:1] S1: release %llu @ sec=%lf\n", S1Cnt, current_time);
    }
...
void *Service_2(void *threadp)
{
    ...
    current_time = getTimeMsec();
    syslog(LOG_CRIT, "[COURSE:1][ASSIGNMENT:1] S2: start on cpu=%d @ sec=%lf\n", sched_getcpu(), current_time);
    ...
    while (!abortS2)
    {
        ...
        current_time = getTimeMsec();
        syslog(LOG_CRIT, "[COURSE:1][ASSIGNMENT:1] S2: release %llu @ sec=%lf\n", S2Cnt, current_time);
    }
...
void *Service_3(void *threadp)
{
    ...
    current_time = getTimeMsec();
    syslog(LOG_CRIT, "[COURSE:1][ASSIGNMENT:1] S3: start on cpu=%d @ sec=%lf\n", sched_getcpu(), current_time);
    ...
    while (!abortS3)
    {
        ...
        current_time = getTimeMsec();
        syslog(LOG_CRIT, "[COURSE:1][ASSIGNMENT:1] S3: release %llu @ sec=%lf\n", S3Cnt, current_time);
    }
...
void main(void)
{
    ...
    FILE *syslog_file = fopen("assignment1_syslog.txt", "w");
    if (syslog_file == NULL) {
        perror("Failed to open assignment1_syslog.txt");
        exit(EXIT_FAILURE);
    }
    ...
    if (fp != NULL)
    {
        ...
        {
            syslog(LOG_CRIT, "[COURSE:1][ASSIGNMENT:1] System Info: %s", sys_info);
        }
        pclose(fp);
    }
