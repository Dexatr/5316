...
void *thread_function2(void *tid) {
    int thread_id = *(int *)tid, period = 5, compute_time = 1;  // Set period and WCET for S2
    struct timespec next_run;  // Structure to hold the next activation time
    clock_gettime(CLOCK_MONOTONIC, &next_run);  // Get the current time in monotonic clock
    time_t end_time = time(NULL) + CYCLE_PERIOD;  // Calculate when the thread should stop running
...
// Thread function for the third service, which simulates a task every 7ms (14.29Hz)
void *thread_function3(void *tid) {
    int thread_id = *(int *)tid, period = 7, compute_time = 1;  // Set period and WCET for S3
    struct timespec next_run;  // Structure to hold the next activation time
    clock_gettime(CLOCK_MONOTONIC, &next_run);  // Get the current time in monotonic clock
    time_t end_time = time(NULL) + CYCLE_PERIOD;  // Calculate when the thread should stop running
...
// Thread function for the fourth service, which simulates a task every 13ms (7.69Hz)
void *thread_function4(void *tid) {
    int thread_id = *(int *)tid, period = 13, compute_time = 2;  // Set period and WCET for S4
    struct timespec next_run;  // Structure to hold the next activation time
    clock_gettime(CLOCK_MONOTONIC, &next_run);  // Get the current time in monotonic clock
    time_t end_time = time(NULL) + CYCLE_PERIOD;  // Calculate when the thread should stop running
...
int main() {
    pthread_t threads[4];  // Array to hold thread identifiers for 4 services
