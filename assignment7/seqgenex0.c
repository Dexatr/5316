...
// Thread function for the first service, which simulates a task every 3ms (33.33Hz)
void *thread_function1(void *tid) {
    int thread_id = *(int *)tid, period = 3, compute_time = 1;  // Set period and WCET for S1
...
// Thread function for the second service, which simulates a task every 6ms (16.67Hz)
void *thread_function2(void *tid) {
    int thread_id = *(int *)tid, period = 6, compute_time = 2;  // Set period and WCET for S2
    struct timespec next_run;  // Structure to hold the next activation time
    clock_gettime(CLOCK_MONOTONIC, &next_run);  // Get the current time in monotonic clock
    time_t end_time = time(NULL) + CYCLE_PERIOD;  // Calculate when the thread should stop running
...
// Thread function for the third service, which simulates a task every 9ms (11.11Hz)
void *thread_function3(void *tid) {
    int thread_id = *(int *)tid, period = 9, compute_time = 3;  // Set period and WCET for S3
    struct timespec next_run;  // Structure to hold the next activation time
    clock_gettime(CLOCK_MONOTONIC, &next_run);  // Get the current time in monotonic clock
    time_t end_time = time(NULL) + CYCLE_PERIOD;  // Calculate when the thread should stop running
